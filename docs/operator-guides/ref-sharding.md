...
A pre-M12 binary reading a v2 manifest fails with `ErrUnsupportedSchema` from the `SchemaGate` check. This is fail-closed — there is no silent misinterpretation hazard. Operators with mixed-version fleets must upgrade every binary that touches a given repo before resharding it. The schema bump is global — every repo created by an M12+ build emits `schema_version: 2`, even if it never uses sharded refs. Pre-M12 binaries cannot read any M12-created repo.
