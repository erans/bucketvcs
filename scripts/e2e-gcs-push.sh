...
# === 6. Verify: ls-remote + round-trip clone ================================
step "Verifying refs on the server"
LS="$(git_ni ls-remote "$REMOTE")"
printf '%s\n' "$LS" | grep -q "refs/heads/$BRANCH" || die "pushed branch not visible via ls-remote"
info "$(printf '%s\n' "$LS" | wc -l | tr -d ' ') refs advertised"
ok "branch refs/heads/$BRANCH present on server"

step "Round-trip: cloning back from bucketvcs"
RT="$WORK/roundtrip"
git_ni clone --quiet "$REMOTE" "$RT"
RT_HEAD="$(git -C "$RT" rev-parse HEAD)"
[ "$RT_HEAD" = "$SRC_HEAD" ] || die "round-trip HEAD mismatch: source=$SRC_HEAD bucketvcs=$RT_HEAD"
ok "round-trip HEAD matches source ($RT_HEAD)"

# Deeper check: full commit-graph parity on the pushed branch.
...
ok "commit count matches ($SRC_LOG commits)"
